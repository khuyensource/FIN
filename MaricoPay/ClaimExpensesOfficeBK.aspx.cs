...
namespace MaricoPay
{
    public partial class ClaimExpensesOfficeBK : System.Web.UI.Page
    {
        ...
        private bool SaveParent(string code)
        {
            try
            {
                using (var dbs = new DBTableDataContext())
                {
                    ...
                    var model = new ClaimExpense { Code_PK = code, Type = radioClaim.SelectedValue, DateRec = raddateNow.SelectedDate.Value, UserName = Session["username"].ToString(), Approver = txtAppName.Text, AppEmail = txtAppEmail.Text, Status = 0, FDate = raddateFrom.SelectedDate.Value, TDate = raddateTo.SelectedDate.Value, NoDays = TinhNgay(), Purpose = txtPurpose.Text, Market = dropMarket.SelectedValue, Department = comboDepartment1.Values, Position = txtPosition.Text, FullName = txtName.Text, DaTamUng = (decimal)radnumAdvncedAmount.Value, Tra_ThuChenhLech = 0, DocTot = 0, NoteApprover = "", Email = txtMyEmail.Text };
      ...
      //insert
        ...
        private void SenEmailSubmit(string code)
        {
           ...
           // db.ClaimExpenses.InsertAllOnSubmit
            ...
            sys.SendMailASP(to, cc, "Approved Claim", "Please approve Claim No " + code + ". Can you approve by click <a href = '" + Request.Url.AbsoluteUri.Replace("ClaimExpensesOffice.aspx?type=0", "") + "approved.aspx?ActivationCode=" + activationCode.ToString() + "&AppLevel=lv1&code=" + code + "'>Click here to approved.</a></br><a href = '" + Request.Url.AbsoluteUri.Replace("ClaimExpensesOffice.aspx?type=0", "") + "approved.aspx?RejectedCode=" + activationCode.ToString() + "&AppLevel=lv1&code=" + code + "'>Click here to Rejected.</a>");
